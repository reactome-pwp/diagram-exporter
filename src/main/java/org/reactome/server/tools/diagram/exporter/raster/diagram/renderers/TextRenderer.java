...
public class TextRenderer {
    ...
    private static List<String> fit(String text, Font font, Graphics2D graphics, double maxWidth, double maxHeight) {
        ...
        if (computeWidth(text, font, graphics) < maxWidth && computeHeight(1, font, graphics) < maxHeight)
    ...
    private static int computeWidth(String text, Font font, Graphics2D graphics) {
        // From https://stackoverflow.com/a/33287695 to have correct estimation
        ...
        FontMetrics fontMetrics = graphics.getFontMetrics(helperFont);
        return Math.round(fontMetrics.stringWidth(text) * scaling);
    }
