...
public class DefaultFontMapper implements FontMapper {
	...
	public PdfFont awtToPdf(Font font) {
		try {
			...
			if (p != null)
				return PdfFontFactory.createFont(p.fontName, p.encoding, p.embedded, p.cached);
			String fontKey = null;
	 ...
	 *
	 * @param allNames the returned value of calling {@link BaseFont#getAllFontNames(String,
	 *                 String, byte[])}
	...
	/**
	 * Inserts one font file into the map. The encoding will be
	 * <CODE>BaseFont.CP1252</CODE> but can be changed later.
	 *
	...
	public static class BaseFontParameters {
		...
		public BaseFontParameters(String fontName) {
			...
			encoding = PdfEncodings.CP1252;
			embedded = EMBED;
			cached = PdfFont.CACHED;
		}
	}
}
