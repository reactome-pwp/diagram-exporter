...
import org.reactome.server.tools.diagram.exporter.utils.ReactomeObjectUtils;
...
public class LogoRenderer {
    ...
    public static NodeProperties addLogo(DiagramCanvas canvas, RasterArgs args, Diagram diagram) {
        // use dbId to avoid adding logo in PDF report when export a reaction
        boolean pharmGKB = ReactomeObjectUtils.pharmGKBExists(String.valueOf(diagram.getDbId()));
        ...
        if (!args.getWriteTitle()) {
            ...
            if (limits != null) {
                ...
                if (pharmGKB) {
                    final NodeProperties pharmGKBLimits = findPharmGKBLogoPlace(bounds, diagram, logoWidth, logoHeight, limits);
                    canvas.getLogoLayer().add(getPharmGKBLogo(), pharmGKBLimits);
                }
                return limits;
            }
        }
        // This point will be reached only if logo couldn't be written inside the diagram or if writeTitle is true
        // double x = pharmGKB?  bounds.getMinX():bounds.getMaxX() - logoWidth;
        ...
        canvas.getLogoLayer().add(logo, limits);
        if (pharmGKB) {
            canvas.getLogoLayer().add(getPharmGKBLogo(), NodePropertiesFactory.get(
                    bounds.getMinX(),
                    bounds.getMaxY() + LEGEND_TO_DIAGRAM_SPACE,
                    logoWidth, logoHeight));
        }
...
//		createBottomTextBox(logoWidth, logoHeight);
    ...
    private static NodeProperties findPharmGKBLogoPlace(Rectangle2D bounds, Diagram diagram, double width, double height, NodeProperties nodeProperties) {
        ...
        final List<Rectangle2D> positions = Arrays.asList(
                ...
                new Rectangle2D.Double(mx, my + LOGO_PADDING + height, width, height));
        for (Rectangle2D position : positions) {
            if (position.getX() * position.getY() > 0 && position.getY() < bounds.getHeight() && position.getX() < bounds.getWidth()) {
                if (!anyInside(diagram, position)) {
                    return NodePropertiesFactory.get(position.getX(), position.getY(), width, height);
                }
            }
    ...
    private static BufferedImage getPharmGKBLogo() {
        final String filename = "images/PHARMGKB_logo.png";
        final InputStream resource = Resources.class.getResourceAsStream(filename);
        try {
            return ImageIO.read(resource);
        } catch (IOException e) {
