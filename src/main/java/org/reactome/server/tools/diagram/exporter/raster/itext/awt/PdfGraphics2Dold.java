...
public class PdfGraphics2Dold extends Graphics2D {
	...
	protected PdfCanvas cb;

	/** Storage for BaseFont objects created. */
	protected HashMap<String, PdfFont> baseFonts;
	...
	private PdfGraphics2Dold(PdfCanvas canvas) {
		this.cb = canvas;
		setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON);
		setRenderingHint(HyperLinkKey.KEY_INSTANCE, HyperLinkKey.VALUE_HYPERLINKKEY_OFF);
	}

	public PdfGraphics2Dold(PdfCanvas cb, final float width, final float height) {
		this(cb, width, height, null, false, false, 0);
	}

	public PdfGraphics2Dold(PdfCanvas cb, final float width, final float height, final FontMapper fontMapper) {
		this(cb, width, height, fontMapper, false, false, 0);
	}

	public PdfGraphics2Dold(PdfCanvas cb, final float width, final float height, final boolean onlyShapes) {
	 ...
	 */
	public PdfGraphics2Dold(PdfCanvas cb, float width, float height, FontMapper fontMapper, boolean onlyShapes, boolean convertImagesToJPEG, float quality) {
		super();
...
//		this.strokeGState = new PdfGState[256];
	...
	public static com.itextpdf.kernel.colors.Color prepareColor(Color color) {
...
//            drawGlyphVector(this.font.createGlyphVector(getFontRenderContext(), s), x, y);
			...
			if (restoreTextRenderingMode) {
				cb.setTextRenderingMode(PdfCanvasConstants.TextRenderingMode.FILL);
			}
		...
		if (!oldOk || nStroke.getEndCap() != oStroke.getEndCap()) {
			switch (nStroke.getEndCap()) {
				case BasicStroke.CAP_BUTT:
					cb.setLineCapStyle(PdfCanvasConstants.LineCapStyle.BUTT);
					break;
		...
		if (!oldOk || nStroke.getLineJoin() != oStroke.getLineJoin()) {
			switch (nStroke.getLineJoin()) {
				...
				case BasicStroke.JOIN_BEVEL:
					cb.setLineJoinStyle(PdfCanvasConstants.LineJoinStyle.BEVEL);
					break;
				default:
...
//			}
	...
	public Graphics create() {
		PdfGraphics2Dold g2 = new PdfGraphics2Dold();
...
//		g2.cb = this.cb.getDuplicate();
	...
	private PdfFont getCachedBaseFont(Font f) {
		synchronized (baseFonts) {
			PdfFont bf = baseFonts.get(f.getFontName());
			if (bf == null) {
	...
	private synchronized void waitForImage(Image image) {
		if (mediaTracker == null)
			mediaTracker = new MediaTracker(new PdfGraphics2Dold.FakeComponent());
	...
	private static final class Kid {
		final int pos;
		final PdfGraphics2Dold graphics;

		Kid(int pos, PdfGraphics2Dold graphics) {
			this.pos = pos;
